...
if ( function_exists( 'equadio_elementor_is_preview' ) && equadio_elementor_is_preview() ) {
	...
	if ( have_posts() ) {
		...
		if ( isset( $GLOBALS['post'] ) && is_object( $GLOBALS['post'] ) ) {
			equadio_storage_set( 'blog_archive_template_post', $GLOBALS['post'] );
		}
	}

	// Make a new main query
	equadio_new_main_query( array(
		'post_type'      => equadio_get_theme_option( 'post_type' ),
		'category'       => equadio_get_theme_option( 'parent_cat' ),
	...
	get_template_part( apply_filters( 'equadio_filter_get_template_part', equadio_blog_archive_get_template() ) );
}
