		...
		?>>
            ...
            ?><div class="sc_price_item_info"><?php
                if  (!empty($item['title']) || !empty($item['price'])) {
                    ?><div class="sc_price_item_title_wrap"><?php
                        if (!empty($item['title'])) {
                    ...
                    ?></div><!-- /.sc_price_item_title_wrap --><?php
                }
			...
			if (!empty($item['image'])) {
				$item['image'] = trx_addons_get_attachment_url($item['image'], apply_filters('trx_addons_filter_thumb_size', trx_addons_get_thumb_size('wide'), 'price-default'));
				if (!empty($item['image'])) {
					$attr = trx_addons_getimagesize($item['image']);
					?><div class="sc_price_item_image"><img src="<?php echo esc_url($item['image']); ?>" alt="<?php esc_attr_e('Icon', 'equadio'); ?>"<?php echo (!empty($attr[3]) ? ' '.trim($attr[3]) : ''); ?>></div><?php
				}
			} else {
			...
			//Details and link
			?><div class="sc_price_item_details_wrap"><?php
