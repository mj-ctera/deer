...
$svg_bg_dir = equadio_get_svg_from_file(equadio_get_file_dir('images/brushstroke.svg'));
					...
					. (!empty($icon) 
						? '<div class="sc_skills_icon sc_icon_type_' . esc_attr($v['icon_type']) . ' ' . esc_attr($icon) . '">'
								. (!empty($svg)
									...
									: '')
								. (!empty($img)
									? '<img class="sc_icon_as_image" src="'.esc_url($img).'" alt="'.esc_attr__('Icon', 'equadio').'">'
									: '')
								. '</div>'
						: '')
