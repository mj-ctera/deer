 ...
 *
 * @package EQUADIO
 * @since EQUADIO 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'EQUADIO_THEME_FREE' ) ) {
	define( 'EQUADIO_THEME_FREE', false );
}
if ( ! defined( 'EQUADIO_THEME_FREE_WP' ) ) {
	define( 'EQUADIO_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'EQUADIO_THEME_ALLOW_UPDATE' ) ) {
	define( 'EQUADIO_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'EQUADIO_ALLOW_SKINS' ) ) {
	define( 'EQUADIO_ALLOW_SKINS', true );
}
if ( ! defined( 'EQUADIO_DEFAULT_SKIN' ) ) {
	define( 'EQUADIO_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'EQUADIO_REMEMBER_SKIN' ) ) {
	define( 'EQUADIO_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'equadio_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'equadio_importer_set_options', 9 );
	function equadio_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( equadio_get_protocol() . ':' . equadio_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( equadio_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'equadio_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'equadio_ocdi_set_options', 9 );
	function equadio_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( equadio_get_protocol() . ':' . equadio_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'equadio_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'equadio_customizer_theme_setup1', 1 );
	function equadio_customizer_theme_setup1() {
		...
		equadio_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' for the body style in the Theme Options
																		...
																		// (can be removed if filter 'equadio_filter_allow_fullscreen' return false)
