<div class="front_page_section front_page_section_team<?php
	$equadio_scheme = equadio_get_theme_option( 'front_page_team_scheme' );
	if ( ! empty( $equadio_scheme ) && ! equadio_is_inherit( $equadio_scheme ) ) {
		echo ' scheme_' . esc_attr( $equadio_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( equadio_get_theme_option( 'front_page_team_paddings' ) );
	if ( equadio_get_theme_option( 'front_page_team_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$equadio_css      = '';
		$equadio_bg_image = equadio_get_theme_option( 'front_page_team_bg_image' );
...
<?php
	// Add anchor
	$equadio_anchor_icon = equadio_get_theme_option( 'front_page_team_anchor_icon' );
	$equadio_anchor_text = equadio_get_theme_option( 'front_page_team_anchor_text' );
...
?>
	...
	<?php
	if ( equadio_get_theme_option( 'front_page_team_fullheight' ) ) {
	...
	?>
			...
			<?php
			$equadio_css      = '';
			$equadio_bg_mask  = equadio_get_theme_option( 'front_page_team_bg_mask' );
			$equadio_bg_color_type = equadio_get_theme_option( 'front_page_team_bg_color_type' );
			if ( 'custom' == $equadio_bg_color_type ) {
				$equadio_bg_color = equadio_get_theme_option( 'front_page_team_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_team_content_wrap content_wrap">
			<?php
			// Caption
			$equadio_caption = equadio_get_theme_option( 'front_page_team_caption' );
			if ( ! empty( $equadio_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_team_caption front_page_block_<?php echo ! empty( $equadio_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $equadio_caption, 'equadio_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$equadio_description = equadio_get_theme_option( 'front_page_team_description' );
			if ( ! empty( $equadio_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_team_description front_page_block_<?php echo ! empty( $equadio_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $equadio_description ), 'equadio_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_team_output">
				...
				if ( is_active_sidebar( 'front_page_team_widgets' ) ) {
					...
					if ( ! equadio_exists_trx_addons() ) {
						...
						equadio_customizer_need_widgets_message( 'front_page_team_caption', 'ThemeREX Addons - Team' );
					}
				}
				?>
			</div>
