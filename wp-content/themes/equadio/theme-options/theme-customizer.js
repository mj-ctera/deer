/* global equadio_color_schemes, equadio_dependencies, Color */
...
( function( api ) {
	...
	for (var i in equadio_color_schemes) {
		cssTemplate['scheme_' + i] = wp.template( 'equadio-color-scheme-' + i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'equadio-fonts' );
	// Add Template with theme vars
	cssTemplate['theme_vars'] = wp.template( 'equadio-vars' );
	...
	api.bind(
		'ready', function() {
			...
			// Add 'reset' button
			jQuery( '#customize-header-actions #save' ).before( '<input type="button" class="button customize-action-reset" value="' + equadio_customizer_vars['msg_reset'] + '">' );
			jQuery( '#customize-header-actions .customize-action-reset' ).on(
				'click', function(e) {
					if (confirm( equadio_customizer_vars['msg_reset_confirm'] )) {
			...
			if (equadio_customizer_vars && equadio_customizer_vars['theme_name_suffix']) {
				jQuery( '#customize-info .site-title' ).append( equadio_customizer_vars['theme_name_suffix'] );
			}

			// Blur the "load fonts" fields - regenerate options lists in the font-family controls
			jQuery( '#customize-theme-controls [id^="customize-control-load_fonts"]' ).on( 'focusout', equadio_customizer_update_load_fonts );

			// Click on the actions button
			jQuery( '#customize-theme-controls .control-section .customize-control-button input[type="button"]' ).on( 'click', equadio_customizer_actions );

			// Check dependencies in the each section on ready
			jQuery( '#customize-theme-controls .control-section' ).each(
				function () {
					equadio_customizer_check_dependencies( jQuery( this ) );
				}
			);
			...
			jQuery( '#customize-theme-controls .control-section > .accordion-section-title' ).on(
				'click', function() {
					...
					if (id !== '') {
						...
						if (section.length > 0) {
							equadio_customizer_check_dependencies( section );
						}
					}
				}
			);
	...
	api.bind(
		'change', function(obj) {
			...
			equadio_customizer_check_dependencies( jQuery( '#customize-theme-controls #customize-control-' + obj.id ).parents( '.control-section' ) );
			equadio_customizer_refresh_preview( obj );
		}
	);
	...
	api.bind(
		'refresh_schemes', function() {
			...
			for (var i in cssTemplate) {
				...
				i = i.replace( 'scheme_', '' );
				if (typeof equadio_color_schemes[i] === 'undefined') {
					delete cssTemplate['scheme_' + i];
					jQuery( '#tmpl-equadio-color-scheme-' + i ).remove();
				} else if (tpl_idx == '') {
					tpl_idx     = i;
					tpl_content = jQuery( '#tmpl-equadio-color-scheme-' + i ).html().trim();
				}
			}
			// Add new templates
			...
			for (var i in equadio_color_schemes) {
				if (typeof cssTemplate['scheme_' + i] === 'undefined') {
					jQuery( '#tmpl-equadio-color-scheme-' + tpl_idx )
					.clone()
					.attr( 'id', 'tmpl-equadio-color-scheme-' + i )
					.html( tpl_content.replace( regex, '.scheme_' + i ) )
					.insertAfter( jQuery( '#tmpl-equadio-color-scheme-' + tpl_idx ) );
					cssTemplate['scheme_' + i] = wp.template( 'equadio-color-scheme-' + i );
				}
			}
		}
	...
	for (var action in equadio_customizer_vars['actions']['expand']) {
		if (action == 'length') {
			continue;
		}
		if (equadio_customizer_vars['actions']['expand'][action]['type'] == 'panel') {
			api.panel(
				action, function( panel ) {
					panel.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = equadio_customizer_vars['actions']['expand'][panel.id];
			...
			api.section(
				action, function( section ) {
					section.expanded.bind(
						function( isExpanded ) {
							if ( isExpanded ) {
								var data = equadio_customizer_vars['actions']['expand'][section.id];
								if (typeof data['url'] !== 'undefined' && data['url'] !== '') {
									api.previewer.previewUrl.set( data['url'] );
								}
								if (typeof data['callback'] !== 'undefined' && data['callback'] !== '' && typeof window[data['callback']] === 'function') {
	...
	function equadio_customizer_get_field_value(fld) {
	...
	function equadio_customizer_check_dependencies(cont) {
			...
			function() {
				...
				if (depend) {
					...
					for (i in depend) {
						...
						if (fld.length > 0) {
							val = equadio_customizer_get_field_value( fld );
					...
					if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
						ctrl.show().removeClass( 'equadio_options_no_use' );
					} else {
						ctrl.hide().addClass( 'equadio_options_no_use' );
					}
				}
				...
				if (false && id == 'color_scheme') {
					...
					if (fld.length > 0) {
						val     = equadio_customizer_get_field_value( fld );
						var num = 0;
						for (i in equadio_color_schemes) {
							num++;
							if (i == val) {
								break;
							}
	...
	function equadio_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			if (id == 'scheme_storage') {
				...
				// Any color in the scheme_storage is changed
			} else if (Object.values( equadio_sorted_schemes ).indexOf( id ) >= 0) {
				forceUpdateCSS = true;
				...
				if (var_name !== undefined) {
					...
					equadio_customizer_update_theme_vars( jQuery( '[data-customize-setting-link="' + id + '"]' ).data( 'var_name' ), val );
					processed = true;
				}

				// Any theme fonts parameter is changed
			} else {
				for (opt in equadio_theme_fonts) {
					for (rule in equadio_theme_fonts[opt]) {
						if (opt + '_' + rule == id) {
							// Store new value to the fonts table
							equadio_customizer_update_theme_fonts( opt, rule, val );
							processed = true;
							break;
						}
					}
	...
	// Actions buttons
	function equadio_customizer_actions(e) {
	...
	function equadio_customizer_update_theme_vars(opt, value) {
		equadio_theme_vars[opt] = parseFloat( value );
	}

	// Store new value in the theme fonts
	function equadio_customizer_update_theme_fonts(opt, rule, value) {
		equadio_theme_fonts[opt][rule] = value;
	}

	// Change theme fonts options if load fonts is changed
	function equadio_customizer_update_load_fonts() {
		var opt_list = [], i, tag, sel, opt, name = '', family = '', val = '', new_val = '', sel_idx = 0;
		updateCSS    = false;
		for (i = 1; i <= equadio_customizer_vars['max_load_fonts']; i++) {
	...
	function equadio_customizer_update_css() {
		...
		var vars = equadio_customizer_add_theme_vars( equadio_theme_vars );

		// Make styles and add its to the css
		css += equadio_customizer_prepare_html_value( cssTemplate['theme_vars']( vars ) );

		// Add theme specific fonts rules
		var fonts = equadio_customizer_add_theme_fonts( equadio_theme_fonts );

		// Make styles and add its to the css
		css += equadio_customizer_prepare_html_value( cssTemplate['theme_fonts']( fonts ) );

		// Add colors
		var sorted = {}, scheme = '';
		for (var param in equadio_sorted_schemes) {
			if (param == 'length') {
				continue;
			}
			scheme = api( equadio_sorted_schemes[param] )();
			if (scheme && scheme !== 'inherit') {
				sorted[scheme] = 1;
			}
		}
		for (scheme in equadio_color_schemes) {
			sorted[scheme] = 1;
		}
		for (scheme in sorted) {
			var i, colors = [];
			// Copy all colors to the temp var
			for (i in equadio_color_schemes[scheme].colors) {
				colors[i] = equadio_color_schemes[scheme].colors[i];
			}
			...
			var tmpl = jQuery( '#tmpl-equadio-color-scheme-' + scheme ).html().trim();
			for (i in colors) {
	...
	function equadio_customizer_add_theme_colors(colors) {
		if (equadio_additional_colors) {
			var clr = '', v = '';
			for (var k in equadio_additional_colors) {
				v   = equadio_additional_colors[k];
				clr = colors[v['color']];
		...
		for (var tag in fonts) {
			...
			rez[tag + '_font-size']       = typeof fonts[tag]['font-size'] != 'undefined'
									&& fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + equadio_customizer_prepare_css_value( fonts[tag]['font-size'] ) + ";"
												: '';
			...
			rez[tag + '_margin-top']      = typeof fonts[tag]['margin-top'] != 'undefined'
									...
									&& fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + equadio_customizer_prepare_css_value( fonts[tag]['margin-top'] ) + ";"
												: '';
			rez[tag + '_margin-bottom']   = typeof fonts[tag]['margin-bottom'] != 'undefined'
									&& fonts[tag]['margin-bottom'] !== ''
									&& fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + equadio_customizer_prepare_css_value( fonts[tag]['margin-bottom'] ) + ";"
												: '';
		}
		return rez;
	}
	...
	function equadio_customizer_add_theme_vars(vars) {
		...
		for (var i in vars) {
			if (i == 'rad') {
				...
				rez['rad']    = equadio_customizer_prepare_css_value( vars['rad'] );
			...
			} else if (['page', 'sidebar', 'sidebar_gap'].indexOf( i ) >= 0) {
				...
				rez['page']              = equadio_customizer_prepare_css_value( vars['page'] );
				rez['page_boxed']        = equadio_customizer_prepare_css_value( vars['page'] + 2 * vars['page_boxed_extra'] );
				rez['page_fullwide_max'] = equadio_customizer_prepare_css_value( vars['page_fullwide_max'] );
				rez['paddings_fullwide'] = equadio_customizer_prepare_css_value( vars['paddings_fullwide'] );
				rez['sidebar']           = equadio_customizer_prepare_css_value( vars['sidebar'] );
				rez['sidebar_gap']       = equadio_customizer_prepare_css_value( vars['sidebar_gap'] );
				rez['content']           = equadio_customizer_prepare_css_value( vars['page'] - vars['sidebar'] - vars['sidebar_gap'] );
				rez['koef_narrow']       = 3 / 4;
				rez['content_narrow']    = equadio_customizer_prepare_css_value( ( vars['page'] - vars['sidebar'] - vars['sidebar_gap'] ) * rez['koef_narrow'] );
				rez['padding_narrow']    = equadio_customizer_prepare_css_value( ( vars['page'] - vars['sidebar'] - vars['sidebar_gap'] ) * ( 1 - rez['koef_narrow'] ) );
				rez['sidebar_and_gap']   = equadio_customizer_prepare_css_value( vars['sidebar'] + vars['sidebar_gap'] );
				rez['sidebar_prc']       = vars['sidebar'] / vars['page'];
				rez['sidebar_gap_prc']   = vars['sidebar_gap'] / vars['page'];
				rez['grid_gap']          = equadio_customizer_prepare_css_value( vars['grid_gap'] );
				rez['page_1_1']          = equadio_customizer_prepare_css_value( vars['page'] );
				rez['page_1_2']          = equadio_customizer_prepare_css_value( vars['page'] / 2 );
				rez['page_1_3']          = equadio_customizer_prepare_css_value( vars['page'] / 3 );
				rez['page_2_3']          = equadio_customizer_prepare_css_value( vars['page'] / 3 * 2 );
				rez['page_1_4']          = equadio_customizer_prepare_css_value( vars['page'] / 4 );
				rez['page_3_4']          = equadio_customizer_prepare_css_value( vars['page'] / 4 * 3 );
				...
				for (var m in margins) {
					rez['page' + margins[m]]    = equadio_customizer_prepare_css_value( vars['page'] + margins[m] );
					rez['content' + margins[m]] = equadio_customizer_prepare_css_value( vars['page'] - vars['sidebar_gap'] - vars['sidebar'] + margins[m] );
					rez['elm_gap_' + m]         = equadio_customizer_prepare_css_value( margins[m] );
				}
	...
	// Add ed to css value
	function equadio_customizer_prepare_css_value(val) {
		if (val !== '' && val != 'inherit') {
			var ed = ('' + val).substr( -1 );
			if ('0' <= ed && ed <= '9') {
				val += 'px';
	...
	function equadio_customizer_prepare_html_value(val) {
