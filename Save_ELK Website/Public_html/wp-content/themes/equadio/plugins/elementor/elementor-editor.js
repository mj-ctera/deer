...
jQuery( document ).ready(
	function() {
		...
		equadio_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		if (window.elementor !== undefined) {
			...
			jQuery( '#elementor-panel' )
				.on( 'input change', '[data-setting*="equadio_options_"]', function (e) {
					...
					var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.equadio_post_editor' );
					if ( link.length > 0 ) {
						equadio_change_post_edit_link_elementor( link );
					}
				} )
				.on( 'click', '.equadio_post_editor', function(e) {
					equadio_change_post_edit_link_elementor( jQuery(this) );
					if (jQuery(this).hasClass('equadio_hidden' )) {
						e.preventDefault();
						return false;
					}
				});
		...
		function equadio_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
