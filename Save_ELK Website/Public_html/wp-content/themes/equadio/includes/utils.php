...
if ( ! function_exists( 'equadio_array_get_first' ) ) {
	function equadio_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'equadio_array_get_keys_by_value' ) ) {
	function equadio_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'equadio_array_delete_by_value' ) ) {
	function equadio_array_delete_by_value( $arr, $value ) {
		do {
...
if ( ! function_exists( 'equadio_array_from_list' ) ) {
	function equadio_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'equadio_array_slice' ) ) {
	function equadio_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'equadio_array_merge' ) ) {
	function equadio_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'equadio_array_insert_after' ) ) {
	function equadio_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = equadio_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = equadio_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );
...
// @param mixed $stuff one or more arrays or scalars to be inserted into $haystack
// @return int the index at which $needle was found
if ( ! function_exists( 'equadio_array_insert_before' ) ) {
	function equadio_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'equadio_get_tag' ) ) {
	function equadio_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'equadio_get_tag_attrib' ) ) {
	function equadio_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'equadio_get_css_position_from_values' ) ) {
	function equadio_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( equadio_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'equadio_prepare_css_value' ) ) {
	function equadio_prepare_css_value( $val ) {
		if ( '' != $val ) {
...
if ( ! function_exists( 'equadio_parse_icons_classes' ) ) {
	function equadio_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
		$file = equadio_fga( $css );
...
if ( ! function_exists( 'equadio_stripslashes' ) ) {
	function equadio_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
// Get GET, POST value
if ( ! function_exists( 'equadio_get_value_gp' ) ) {
	function equadio_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'equadio_get_value_gpc' ) ) {
	function equadio_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'equadio_get_value_gps' ) ) {
	function equadio_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'equadio_get_session_value' ) ) {
	function equadio_get_session_value( $name, $defa = '' ) {
		global $wp_session;
...
if ( ! function_exists( 'equadio_set_session_value' ) ) {
	function equadio_set_session_value( $name, $value ) {
		global $wp_session;
...
if ( ! function_exists( 'equadio_del_session_value' ) ) {
	function equadio_del_session_value( $name ) {
		global $wp_session;
...
if ( ! function_exists( 'equadio_hex2rgb' ) ) {
	function equadio_hex2rgb( $hex ) {
...
if ( ! function_exists( 'equadio_hex2rgba' ) ) {
	function equadio_hex2rgba( $hex, $alpha ) {
		$rgb = equadio_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'equadio_hex2hsb' ) ) {
	function equadio_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = equadio_rgb2hsb( equadio_hex2rgb( $hex ) );
...
if ( ! function_exists( 'equadio_rgb2hsb' ) ) {
	function equadio_rgb2hsb( $rgb ) {
		$hsb      = array();
...
if ( ! function_exists( 'equadio_hsb2rgb' ) ) {
	function equadio_hsb2rgb( $hsb ) {
		$rgb = array();
		$h   = round( $hsb['h'] );
...
if ( ! function_exists( 'equadio_rgb2hex' ) ) {
	function equadio_rgb2hex( $rgb ) {
		$hex = array(
			dechex( $rgb['r'] ),
			dechex( $rgb['g'] ),
			dechex( $rgb['b'] ),
...
if ( ! function_exists( 'equadio_hsb2hex' ) ) {
	function equadio_hsb2hex( $hsb ) {
		return equadio_rgb2hex( equadio_hsb2rgb( $hsb ) );
	}
}
...
if (!function_exists('equadio_date_to_sql')) {
	function equadio_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
// Display price
if (!function_exists('equadio_format_price')) {
	function equadio_format_price($price) {
...
if (!function_exists('equadio_num2size')) {
	function equadio_num2size($num) {
...
if (!function_exists('equadio_size2num')) {
	function equadio_size2num($size) {
...
if (!function_exists('equadio_parse_num')) {
	function equadio_parse_num($str) {
...
if ( ! function_exists( 'equadio_prepare_macros' ) ) {
	function equadio_prepare_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '<i>', '</i>', '<b>', '</b>', '<br>' ),
			$str
...
if ( ! function_exists( 'equadio_remove_macros' ) ) {
	function equadio_remove_macros( $str ) {
		return str_replace(
			array( '{{', '}}', '((', '))', '||' ),
			array( '', '', '', '', ' ' ),
			$str
...
if ( ! function_exists( 'equadio_is_on' ) ) {
	function equadio_is_on( $prm ) {
		return is_array( $prm )
					? count( $prm ) > 0
					: $prm > 0 || in_array( strtolower( $prm ), array( 'true', 'on', 'yes', 'show' ) );
	}
}
if ( ! function_exists( 'equadio_is_off' ) ) {
	function equadio_is_off( $prm ) {
		return is_array( $prm )
					? count( $prm ) == 0
					: empty( $prm ) || 0 === $prm || in_array( strtolower( $prm ), array( 'false', 'off', 'no', 'none', 'hide' ) );
	}
}
if ( ! function_exists( 'equadio_is_inherit' ) ) {
	function equadio_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

// Return truncated string (by chars number)
if ( ! function_exists( 'equadio_strshort' ) ) {
	function equadio_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = equadio_strip_tags( $str );
...
if ( ! function_exists( 'equadio_strip_tags' ) ) {
	function equadio_strip_tags( $str ) {
...
if ( ! function_exists( 'equadio_excerpt' ) ) {
	function equadio_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return equadio_strwords( equadio_strip_tags( $str ), $maxlength, $add );
	}
}


// Return truncated string (by words number)
if ( ! function_exists( 'equadio_strwords' ) ) {
	function equadio_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
// Unserialize string
if ( ! function_exists( 'equadio_unserialize' ) ) {
	function equadio_unserialize( $str ) {
...
if ( ! function_exists( 'equadio_str_replace' ) ) {
	function equadio_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = equadio_str_replace( $from, $to, $v );
			}
			...
			$str = equadio_unserialize( $str );
			$str = is_array( $str ) || is_object( $str )
							? serialize( equadio_str_replace( $from, $to, $str ) )
...
if ( ! function_exists( 'equadio_str_replace_once' ) ) {
	function equadio_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'equadio_tags_count' ) ) {
	function equadio_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'equadio_getimagesize' ) ) {
	function equadio_getimagesize( $url ) {
		// Remove scheme from url
		$url = equadio_remove_protocol_from_url( $url );
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => equadio_remove_protocol_from_url( $upload_info['baseurl'] ),
			),
			'child'   => array(
				'dir' => EQUADIO_CHILD_DIR,
				'url' => equadio_remove_protocol_from_url( EQUADIO_CHILD_URL ),
			),
			'theme'   => array(
				'dir' => EQUADIO_THEME_DIR,
				'url' => equadio_remove_protocol_from_url( EQUADIO_THEME_URL ),
			),
		);
...
if ( ! function_exists( 'equadio_clear_thumb_size' ) ) {
	function equadio_clear_thumb_size( $url ) {
		$pi            = pathinfo( $url );
		$pi['dirname'] = equadio_remove_protocol_from_url( $pi['dirname'], false );
...
if ( ! function_exists( 'equadio_add_thumb_size' ) ) {
	function equadio_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
			...
			if ( empty( $_wp_additional_image_sizes[ $thumb_size ]['height'] ) || empty( $_wp_additional_image_sizes[ $thumb_size ]['crop'] ) ) {
				...
				if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
					...
					if ( ! empty( $attach[0] ) ) {
						...
						$pi['dirname'] = equadio_remove_protocol_from_url( $pi['dirname'] );
						$parts = explode( '-', $pi['filename'] );
					}
				}
			} else {
		...
		$new_url = equadio_remove_protocol_from_url( $pi['dirname'] . '/' . $pi['filename'] . '.' . $pi['extension'] );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = equadio_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = equadio_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = equadio_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'equadio_get_thumb_size' ) ) {
	function equadio_get_thumb_size( $ts ) {
		$retina = equadio_get_retina_multiplier() > 1 ? '-@retina' : '';
		return ( 'post-thumbnail' == $ts ? '' : 'equadio-thumb-' ) . $ts . $retina;
	}
}
...
if ( ! function_exists( 'equadio_get_closest_thumb_size' ) ) {
	function equadio_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'equadio_get_attachment_url' ) ) {
	function equadio_get_attachment_url( $image_id, $size = 'full' ) {
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			$attach   = wp_get_attachment_image_src( $image_id, $size );
			$image_id = isset( $attach[0] ) && '' != $attach[0] ? $attach[0] : '';
		} else {
			$image_id = equadio_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}
...
if ( ! function_exists( 'equadio_get_post_image' ) ) {
	function equadio_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'equadio_get_post_audio' ) ) {
	function equadio_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = equadio_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = equadio_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = equadio_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( equadio_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = equadio_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'equadio_get_post_audio_list' ) ) {
	function equadio_get_post_audio_list( $args=array() ) {
...
if ( ! function_exists( 'equadio_get_post_audio_list_first' ) ) {
	function equadio_get_post_audio_list_first( $src = false, $args = array() ) {
...
if ( ! function_exists( 'equadio_get_post_video' ) ) {
	function equadio_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = equadio_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = equadio_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = equadio_get_tag( $post_text, '<video', '</video>' );
				if ( empty( $img ) ) {
					$sc = equadio_get_tag( $post_text, '[video', '[/video]' );
					if ( empty( $sc ) ) {
						$sc = equadio_get_tag( $post_text, '[trx_widget_video', '' );
					}
					if ( ! empty( $sc ) ) {
						$img = do_shortcode( $sc );
					}
					if ( empty( $img ) && preg_match_all( '/<!\\-\\- wp\\:trx-addons\\/video.+"link"\\:"([^"]*)"/i', $post_text, $matches ) ) {
						$img = equadio_get_embed_video( $matches[1][0] );
					}
					if ( empty( $img ) && preg_match_all( '/<!\\-\\- wp:core-embed\\/(youtube|vimeo|dailymotion|facebook).+"url":"([^"]*)"/i', $post_text, $matches ) ) {
						$img = equadio_get_embed_video( $matches[2][0], true );
					}
		...
		return apply_filters( 'equadio_filter_get_post_video', $img );
	}
}


// Return list videos from post attributes
if ( ! function_exists( 'equadio_get_post_video_list' ) ) {
	function equadio_get_post_video_list( $args=array() ) {
...
if ( ! function_exists( 'equadio_get_post_video_list_first' ) ) {
	function equadio_get_post_video_list_first( $src = false, $args = array() ) {
...
if ( ! function_exists( 'equadio_get_post_iframe' ) ) {
	function equadio_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = equadio_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = equadio_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'equadio_filter_get_post_iframe', $img );
	}
}


// Add 'autoplay' feature in the video
if ( ! function_exists( 'equadio_make_video_autoplay' ) ) {
	function equadio_make_video_autoplay( $video ) {
		if ( strpos( $video, '<video' ) !== false ) {
			...
			if ( false !== $pos ) {
				if ( preg_match( '/(<' . esc_html( $tag ) . '.+src=[\'"])([^\'"]+)([\'"][^>]*>)(.*)/i', $video, $matches ) ) {
					$video = ( $pos > 0 ? substr( $video, 0, $pos ) : '' ) . $matches[1] . $matches[2];
					if ( ! empty( $matches[2] ) && strpos( $matches[2], 'autoplay=1' ) === false ) {
						$video .= ( strpos( $matches[2], '?' ) !== false ? '&' : '?' ) . 'autoplay=1';
					}
...
if ( ! function_exists( 'equadio_get_embed_video' ) ) {
	function equadio_get_embed_video( $video, $use_wp_embed = false ) {
		global $wp_embed;
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			$embed_video = do_shortcode( $wp_embed->run_shortcode( '[embed]' . trim( $video ) . '[/embed]' ) );
			$embed_video = equadio_make_video_autoplay( $embed_video );
		} else {
			$video = str_replace( '/watch?v=', '/embed/', $video );
			$video = equadio_add_to_url(
				$video,
				array(
					'feature'        => 'oembed',
					'controls'       => 0,
			...
			$tag = equadio_get_embed_video_tag_name();
			$embed_video  = '<' . esc_html( $tag ) . ' src="' . esc_url( $video ) . '" allow="autoplay" width="1170" height="658" frameborder="0"></' . esc_html( $tag ) . '>';
		}
		return $embed_video;
	}
}
...
// Return layout with embeded video
if ( ! function_exists( 'equadio_get_embed_video_tag_name' ) ) {
	function equadio_get_embed_video_tag_name() {
...
if ( ! function_exists( 'equadio_is_from_uploads' ) ) {
	function equadio_is_from_uploads( $url ) {
		$url          = equadio_remove_protocol_from_url( $url );
		$uploads_info = wp_upload_dir();
		$uploads_url  = equadio_remove_protocol_from_url( $uploads_info['baseurl'] );
		$uploads_dir  = $uploads_info['basedir'];
...
if ( ! function_exists( 'equadio_is_youtube_url' ) ) {
	function equadio_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

// Check if URL from Vimeo
if ( ! function_exists( 'equadio_is_vimeo_url' ) ) {
	function equadio_is_vimeo_url( $url ) {
...
------------------------------------------------------------------- */
if ( ! function_exists( 'equadio_init_filesystem' ) ) {
	add_action( 'after_setup_theme', 'equadio_init_filesystem', 0 );
	function equadio_init_filesystem() {
...
if ( ! function_exists( 'equadio_prepare_path' ) ) {
	function equadio_prepare_path( $path ) {
...
if ( ! function_exists( 'equadio_fpc' ) ) {
	function equadio_fpc( $file, $data, $flag = 0 ) {
		...
		if ( ! empty( $file ) ) {
			if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
				...
				if ( equadio_is_on( equadio_get_theme_option( 'debug_mode' ) ) ) {
					...
					throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Put contents to the file "%s" failed', 'equadio' ), $file ) );
				}
			}
		}
		return false;
...
if ( ! function_exists( 'equadio_fgc' ) ) {
	function equadio_fgc( $file ) {
		global $wp_filesystem;
		if ( ! empty( $file ) ) {
			if ( function_exists( 'trx_addons_fgc' ) ) {
				...
				if ( equadio_is_on( equadio_get_theme_option( 'debug_mode' ) ) ) {
					// Translators: Add the file name to the message
					throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get contents from the file "%s" failed', 'equadio' ), $file ) );
				}
			}
		}
		return '';
...
if ( ! function_exists( 'equadio_fga' ) ) {
	function equadio_fga( $file ) {
		global $wp_filesystem;
		if ( ! empty( $file ) ) {
			if ( isset( $wp_filesystem ) && is_object( $wp_filesystem ) ) {
				$file = str_replace( ABSPATH, $wp_filesystem->abspath(), $file );
				...
				if ( equadio_is_on( equadio_get_theme_option( 'debug_mode' ) ) ) {
					// Translators: Add the file name to the message
					throw new Exception( sprintf( esc_html__( 'WP Filesystem is not initialized! Get rows from the file "%s" failed', 'equadio' ), $file ) );
				}
			}
		}
		return array();
...
if ( ! function_exists( 'equadio_retrieve_json' ) ) {
	function equadio_retrieve_json( $url ) {
		return function_exists( 'trx_addons_retrieve_json' ) ? trx_addons_retrieve_json( $url ) : '';
	}
}

// Remove unsafe characters from file/folder path
if ( ! function_exists( 'equadio_esc' ) ) {
	function equadio_esc( $name ) {
...
if ( ! function_exists( 'equadio_check_min_file' ) ) {
	function equadio_check_min_file( $file, $dir ) {
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && equadio_is_off( equadio_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && equadio_is_off( equadio_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'equadio_get_file_dir' ) ) {
	function equadio_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! EQUADIO_ALLOW_SKINS && ! equadio_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'equadio_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( EQUADIO_CHILD_DIR != EQUADIO_THEME_DIR && file_exists( EQUADIO_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? EQUADIO_CHILD_URL : EQUADIO_CHILD_DIR ) . equadio_check_min_file( $file, EQUADIO_CHILD_DIR );
			} elseif ( file_exists( EQUADIO_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? EQUADIO_THEME_URL : EQUADIO_THEME_DIR ) . equadio_check_min_file( $file, EQUADIO_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'equadio_get_file_url' ) ) {
	function equadio_get_file_url( $file ) {
		return equadio_get_file_dir( $file, true );
	}
}

// Return file extension from full name/path
if ( ! function_exists( 'equadio_get_file_ext' ) ) {
	function equadio_get_file_ext( $file ) {
...
if ( ! function_exists( 'equadio_get_file_name' ) ) {
	function equadio_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
...
if ( ! function_exists( 'equadio_get_folder_dir' ) ) {
	function equadio_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'equadio_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( EQUADIO_CHILD_DIR != EQUADIO_THEME_DIR && is_dir( EQUADIO_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? EQUADIO_CHILD_URL : EQUADIO_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( EQUADIO_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? EQUADIO_THEME_URL : EQUADIO_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'equadio_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'equadio_get_folder_url' ) ) {
	function equadio_get_folder_url( $folder ) {
		return equadio_get_folder_dir( $folder, true );
	}
}


// Merge all separate styles and scripts to the single file to increase page upload speed
if ( ! function_exists( 'equadio_merge_js' ) ) {
	function equadio_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= equadio_fgc( equadio_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			equadio_fpc(
				equadio_get_file_dir( $to ),
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'equadio' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'equadio_filter_js_output', apply_filters( 'equadio_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}
...
if ( ! function_exists( 'equadio_merge_css' ) ) {
	function equadio_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'equadio_filter_responsive_sizes', equadio_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = equadio_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = equadio_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'equadio_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'equadio_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'equadio' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'equadio' ) );
		...
		if ( '' != $output ) {
			equadio_fpc(
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'equadio') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'equadio_filter_css_output', apply_filters( 'equadio_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}
...
// Merge styles to the CSS file
if ( ! function_exists( 'equadio_filter_merge_css' ) ) {
	add_filter( 'equadio_filter_merge_css', 'equadio_filter_merge_css' );
	function equadio_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

// Merge styles to the SASS file
if ( ! function_exists( 'equadio_merge_sass' ) ) {
	function equadio_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'equadio_filter_responsive_sizes', equadio_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'equadio_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			equadio_fpc( equadio_get_file_dir( $to ), apply_filters( 'equadio_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'equadio_remove_protocol_from_url' ) ) {
	function equadio_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

// Add parameters to URL
if ( ! function_exists( 'equadio_add_to_url' ) ) {
	function equadio_add_to_url( $url, $prm ) {
